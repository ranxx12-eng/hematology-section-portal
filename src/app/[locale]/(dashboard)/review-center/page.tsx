...
export default function ReviewCenterPage() {
  ...
  const canAccess = user?.role ? canReviewTasks(user.role, can) : false;
  ...
  const load = useCallback(async () => {
    ...
    if (tasksResult.error) {
      ...
      const pending = filterTasksForReviewCenter(tasksResult.data, user?.employeeId);
      setItems(pending.map((t) => taskToReviewQueueItem(t, nameMap, locale)));
    }
    setLoading(false);
  }, [locale, user?.employeeId]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Records waiting for your review</p>
        </div>
