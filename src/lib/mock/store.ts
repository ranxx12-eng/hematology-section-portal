...
function createQCRecords(instruments: Instrument[]): QCRecord[] {
  return Array.from({ length: 30 }, (_, i) => {
    ...
    return {
      ...
      qcFrequency: (['daily', 'monthly'] as const)[i % 7 === 0 ? 1 : 0],
      qcStatus,
      correctiveActions: qcStatus === 'OUT' ? ['Repeat QC'] : [],
      resolutionStatus,
      performedByName: 'Demo Tech',
      performedByStaffId: 'HEM-0001',
      reviewStatus: i % 4 === 0 ? 'Reviewed' : 'Pending Review',
      approvalStatus: i % 8 === 0 ? 'Approved' : 'Pending Approval',
      createdAt: now,
      updatedAt: now,
    };
  });
...
export function createMockDatabase(): MockDatabase {
  ...
  return {
    ...
    correctedResults: Array.from({ length: 10 }, (_, i) => ({
      ...
      date: daysAgo(i % 30),
      patientName: `Demo Patient ${i + 1}`,
      patientId: `DEMO-P${String(4000 + i).padStart(6, '0')}`,
      labAccession: `ACC-${String(1000 + i)}`,
      test: ['CBC', 'PT', 'INR'][i % 3],
      originalResult: String(10 + i),
      correctedResult: String(12 + i),
      reason: 'Transcription error',
      status: (['Open', 'Completed', 'Pending Review'] as const)[i % 3],
      correctedBy: 'emp-005',
