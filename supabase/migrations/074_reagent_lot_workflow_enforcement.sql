-- ============================================================================
-- Migration 074: Database-enforced reagent lot study workflow
-- Blocks direct status/workflow field updates; routes transitions through RPC.
-- Uses existing inventory.manage permission only.
-- ============================================================================
...
BEGIN
  ...
  IF workflow_changed
    AND current_setting('app.reagent_lot_workflow_bypass', true) IS DISTINCT FROM 'true' THEN
    RAISE EXCEPTION 'Reagent lot study workflow fields can only be changed through workflow actions'
      USING ERRCODE = '42501';
  END IF;

  IF activation_changed
    AND current_setting('app.reagent_lot_activation_bypass', true) IS DISTINCT FROM 'true' THEN
...
DROP TRIGGER IF EXISTS trg_enforce_reagent_lot_workflow ON public.inventory_reagent_lot_comparisons;
CREATE TRIGGER trg_enforce_reagent_lot_workflow
  BEFORE UPDATE ON public.inventory_reagent_lot_comparisons
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_reagent_lot_workflow_update();
...
REVOKE ALL ON FUNCTION public.enforce_reagent_lot_workflow_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.perform_reagent_lot_workflow_action(UUID, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enforce_reagent_lot_workflow_update() TO authenticated;
GRANT EXECUTE ON FUNCTION public.perform_reagent_lot_workflow_action(UUID, TEXT, TEXT) TO authenticated;

COMMIT;

-- Patch activation RPC to set activation bypass for activated_at updates
...
BEGIN
  ...
  PERFORM set_config('app.reagent_lot_activation_bypass', 'true', true);
  UPDATE public.inventory_reagent_lot_comparisons
