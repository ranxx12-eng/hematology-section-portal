-- ============================================================================
-- Migration 050: Environmental Live Monthly Log — public read-only RPC access
-- Narrowly scoped data for QR live monthly logs. Does NOT weaken RLS on base tables.
-- Does NOT auto-apply.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Access log (optional audit of live QR scans)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.env_live_access_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  asset_code TEXT NOT NULL,
  viewed_year INTEGER,
  viewed_month INTEGER,
  viewer_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
  viewed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

ALTER TABLE public.env_live_access_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS env_live_access_log_insert ON public.env_live_access_log;
CREATE POLICY env_live_access_log_insert ON public.env_live_access_log
  FOR INSERT TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS env_live_access_log_select ON public.env_live_access_log;
CREATE POLICY env_live_access_log_select ON public.env_live_access_log
  FOR SELECT TO authenticated
  USING (public.has_permission('environmental.audit'));

-- ---------------------------------------------------------------------------
-- Public RPC: narrowly scoped monthly log payload (no UUIDs in response)
-- ---------------------------------------------------------------------------
...
-- Log live view access (non-blocking for callers)
CREATE OR REPLACE FUNCTION public.log_env_live_access(
  p_asset_code TEXT,
...
AS $$
BEGIN
  INSERT INTO public.env_live_access_log (asset_code, viewed_year, viewed_month, viewer_id)
  VALUES (
...
REVOKE ALL ON FUNCTION public.log_env_live_access(TEXT, INTEGER, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.log_env_live_access(TEXT, INTEGER, INTEGER) TO anon, authenticated;
